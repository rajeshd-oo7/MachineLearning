In this project we build a classifier based on Rajesh Dachiraju's paper ([cited below] (https://doi.org/10.13140/RG.2.2.18109.83682)). The problem in using the method described in this paper is that the number of computations grow exponentially with the increase in feature dimensions. According to the paper "Testing the Manifold Hypothesis", every high dimensional data lies in the vicinity of a low dimensional manifold. So to overcome the high computations problem, before using the method, we do a non linear dimensionality reduction. The method we choose is UMAP.
...
1. Dachiraju, R., 2025. Binary Regression for BV Class Conditional Probability. (https://doi.org/10.13140/RG.2.2.18109.83682)
